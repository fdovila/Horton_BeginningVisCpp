...
CSketcherDoc::CSketcherDoc() : m_Element(ElementType::LINE), m_Color(ElementColor::BLACK), m_PenWidth(0),
...
void CSketcherDoc::OnPenWidth()
{
  ...
  aDlg.m_PenWidth = m_PenWidth;                                        // Set pen width in the dialog to that in the document
...
CRect CSketcherDoc::GetDocExtent()const
{
  ...
  CRect docExtent(m_Sketch.front()->GetEnclosingRect());   // Initial document extent
  for(auto& pElement : m_Sketch)
